...
async function loadImages(){
    for (let index = 0; index < (data.length)-count; index++) {
        if (count<2){
            divisor = ((count%2)+1)
        ...
        newHtml = `
        <figure id="image-`+count+`" onclick="fullscreen(`+count+`)" class="picture-card-container">
    ...
    (async function(){
        ...
        if(data[contentId]["type"]=="youtube_embed"){
            ...
            var player2 = new YT.Player('fullscreen-content', {
                videoId: data[contentId]["fullLink"],
                playerVars: {
                    'playsinline': 1
                },
            })
