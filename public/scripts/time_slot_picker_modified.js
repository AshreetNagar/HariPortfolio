// Source: https://github.com/sanmark/Time-Slot-Picker

// Define select and unselect events
// Create the event. and Define that the event name is 'build'.
const slotSelectedEvent = document.createEvent("Event");
slotSelectedEvent.initEvent("slotSelected", true, true);
...
(function ( $ ) {
    $.fn.timeSlotPicker = function(options) {
        ...
        function formatTime(h,m){
            ...
            timeString = hour + hMSeperator + min + meridian;
            if(settings['showEntireTimeslot']){
                var h1 = h+2;
                if (h1>=12){
                    meridian=" PM";
                    h1 = h1-12;
                ...
                timeString += " - "
                timeString += (h1<10?"0"+h1:h1) + hMSeperator + min + meridian;
            }
            return timeString;
        }
        ...
        function rendertimeSlotPicker(selectedDate){
            ...
            for (let index = startTimeInMins; index <= endTimeInMins;) {
                ...
                if(slotCounter == settings['maxPerRow'] || index == endTimeInMins){
                    slotCounter = 0;
                    slots.push(slotRow);
                    slotRow=[];
                }
            ...
            var element  = '<table class="time-slot-picker-table" style="width: 100%;">';
                ...
                slots.forEach(row => {
                    ...
                    row.forEach(slot => {
                        ...
                        const timeData = formatTime(slot['hours'],slot['minutes']); 
                        
                        if(slot['disable']){
                            element+='<div class="tsp-time-wrapper tsp-time-slot-disabled" style="color:#D3D3D3;cursor: not-allowed;width:fit-content;margin:auto;padding:5px;border-radius:15px;">'+timeData+'</div>';
                        }else{
                            element+='<div class="tsp-time-wrapper" style="color:#4A4B4E;cursor:pointer;width:fit-content;margin:auto;padding:5px;border-radius:15px;">'+timeData+'</div>';
                        }
        ...
        function setInputValue(){
            ...
            // if(settings['inputElementSelector'] && $(settings['inputElementSelector']).length>0 && selectedSlotTime){
                ...
                document.dispatchEvent(slotSelectedEvent);
